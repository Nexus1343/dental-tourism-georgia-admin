...
import { CreateTreatmentPackage, Treatment, Clinic } from '@/types/database'
import { toast } from 'sonner'
import Link from 'next/link'

export default function CreateTreatmentPackagePage() {
  ...
  const [formData, setFormData] = useState<CreateTreatmentPackage>({
    clinic_id: undefined,
    name: '',
    description: '',
    treatment_ids: [],
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('/api/admin/treatment-packages', {
        method: 'POST',
        headers: {
      ...
      if (!response.ok) {
        throw new Error(data.error || 'Failed to create treatment package')
      }
      ...
      toast.error('Failed to create treatment package')
    } finally {
      setLoading(false)
    }
  }

  const updateFormData = (field: keyof CreateTreatmentPackage, value: any) => {
    ...
    <div className="space-y-6 max-w-full overflow-x-hidden">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h1 className="text-2xl font-bold">Create Treatment Package</h1>
            <p className="text-muted-foreground">Set up a new treatment package with multiple treatments</p>
          </div>
        </div>
      </div>
