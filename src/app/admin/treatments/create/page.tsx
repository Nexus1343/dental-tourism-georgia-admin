...
import { TreatmentCategory, CreateTreatment } from '@/types/database'
...
export default function CreateTreatmentPage() {
  ...
  const [formData, setFormData] = useState<CreateTreatment>({
    name: '',
    category: 'preventive',
    description: '',
    procedure_details: '',
  ...
  const updateFormData = (field: keyof CreateTreatment, value: any) => {
    setFormData(prev => ({ ...prev, [field]: value }))
  }

  const addToArray = (field: keyof CreateTreatment, value: string, setter: (value: string) => void) => {
    if (value.trim()) {
  ...
  const removeFromArray = (field: keyof CreateTreatment, index: number) => {
    const currentArray = formData[field] as string[] || []
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-3">
          ...
          <h1 className="text-2xl font-bold">Create New Treatment</h1>
        </div>
      </div>
